...
*/
	class Adafruit_SPITFT : public ardPort::Adafruit_GFX
	{
	  public:
		...
		void constructObject(uint16_t w, uint16_t h, int8_t cs, int8_t dc, int8_t mosi, int8_t sck, int8_t rst, int8_t miso);
		void constructObject(uint16_t w, uint16_t h, SPIClass* spiClass, int8_t cs, int8_t dc, int8_t rst);
		...
		*/
		...
		// １ビットカラーのビットマップ転送
		void drawBWBitmap(int16_t x, int16_t y, uint8_t* pbits, int16_t w, int16_t h);

		// 　１６ビットカラーのビットマップ転送用ラッパー関数を用意する
		...
		// 　８ビットカラーのビットマップ転送用ラッパー関数を用意する
		void drawRGBBitmap(int16_t x, int16_t y, GFXcanvas8* canvas)
		{
			if (canvas->isBackground) {
				// 透過色を使うときは、描画ウインドウを使っての描画はできない（と思う）ので、Adafruitのビットマップを呼びだす
				Adafruit_GFX::drawRGBBitmap(x, y, canvas);
		...
		// 　１ビットカラーのビットマップ転送用ラッパー関数を用意する
		void drawRGBBitmap(int16_t x, int16_t y, GFXcanvas1* canvas)
		{
			if (canvas->isBackground) {
				// 透過色を使うときは、描画ウインドウを使っての描画はできない（と思う）ので、Adafruitのビットマップを呼びだす
				Adafruit_GFX::drawRGBBitmap(x, y, canvas);
			} else {
				drawBWBitmap(x, y, canvas->getBuffer(), canvas->width(), canvas->height());
			}
		}
