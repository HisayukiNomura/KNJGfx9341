...
// 名前空間の使用を宣言する。
// これを挿入することで、Adafruit_GFXと同じ名前のクラスを使用したときに、本ライブラリが
// 使用される。オリジナルのAdafruit_GFXに戻したい場合は、クラス名の変更などは不要で、
// このusingを削除すればよい。
// また、２つのライブラリを同時に使いたい場合、明示的に ardPort::Adafruit_ILI9341のように
// 修飾して使用することができる。
...
/// @param Text 表示したい長い文字列。
/// @param width 表示領域の横幅　（省略時は接続されているデバイスの画面サイズ）
/// @param height 表示領域の縦幅　（省略時は接続されているデバイスの画面サイズ）
/// @return 文字の長さ（バイト数）
...
/// @brief 画面をタッチバッドでタッチするか、一定期間が経過するまで待つ
/// @param tft 画面のインスタンス
/// @param ts タッチパネルのインスタンス
...
void demoBitmap(Adafruit_ILI9341 tft, XPT2046_Touchscreen ts)
{
...

	{
		...
		const char* pictMsg[] = {"ビットマップ表示のデモンストレーション",
								 ...
								 "透過色無しの高速描画です　　　　　　　"};
		const uint16_t* array[] = {pictFuji1, pictFuji3, pictFuji2, pictFuji4};
		int idx = 0;
		int msIdx = 0;
		tft.setCursor(0, 208);
	...
	{                                                     // 円
		...
		for (int i = 0; i < 1000; i++) {
			...
			while (1) {
				...
				if (xy.x + r >= tft.width() || xy.y + r >= tft.height())
					continue;
				if (xy.x - r < 0 || xy.y - r < 0)
					continue;
				break;
			}
